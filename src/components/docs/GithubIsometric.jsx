...
/**
 * 3D isometric visualization of GitHub contribution graph
 * with interactive orbit controls, scroll-driven spin, and responsive layout.
 */
export default function GithubIsometric({
    ...
    fetch(`https://github-contributions-api.jogruber.de/v4/${username}`)
      ...
      .then((data) => {
        ...
        setStats({
          ...
          currentRange:
            current > 0 ? formatDateRange(currentStart, today) : "-",
        });
        setAllDays(recent);
        setLoading(false);
      })
  ...
  useEffect(() => {
    ...
    gridPlane.scale.x = showEmptyDaysRef.current
      ? gridWidthFull
      : gridWidthPacked;
    scene.add(gridPlane);

    const gridLinesGeo = new THREE.PlaneGeometry(
      1,
      7 * STEP,
      Math.max(1, numWeeksFull),
      7
    );
    ...
    gridLines.scale.x = showEmptyDaysRef.current
      ? gridWidthFull
      : gridWidthPacked;
    scene.add(gridLines);
    ...
    const initialZoom = showEmptyDaysRef.current
      ? 2.15
      : Math.max(0.4, numWeeksPacked / 20);
    ...
    const handleScroll = () => {
      ...
      if (Math.abs(delta) > 0) {
        ...
        targetSpeed.current =
          currentDirection.current * baseSpeed - delta * 0.0015;
      }
      lastScrollY.current = window.scrollY;
    };

    const handleOrientation = (event) => {
      if (isMobileRef.current || event.beta === null || event.gamma === null)
        return;
      gyroTarget.current.x = Math.max(
        -0.3,
        Math.min(0.3, (event.beta - 45) * 0.003)
      );
      gyroTarget.current.z = Math.max(
        -0.3,
        Math.min(0.3, event.gamma * 0.003)
      );
    };
    ...
    function onMouseMove(e) {
      ...
      if (hits.length > 0) {
        ...
        setTooltip({
          x: clientX - rect.left,
          y: clientY - rect.top,
          date: hoveredMesh.userData.date,
          count: hoveredMesh.userData.count,
        });
    ...
    function animate() {
      ...
      gridPlane.scale.x +=
        (targetGridWidth - gridPlane.scale.x) * 0.08;
      gridLines.scale.x +=
        (targetGridWidth - gridLines.scale.x) * 0.08;

      meshGroups.forEach((mesh) => {
        const targetX = isFull
          ? mesh.userData.x_full
          : mesh.userData.x_packed;
        const targetZ = isFull
          ? mesh.userData.z_full
          : mesh.userData.z_packed;
        const targetScale = isFull
          ? 1
          : mesh.userData.isPackedActive
          ? 1
          : 0.001;
        ...
        if (mesh === hoveredMesh && mesh.scale.y > 0.5) {
          ...
          mesh.rotation.x +=
            (mesh.userData.baseRotX - mesh.rotation.x) * 0.1;
          mesh.rotation.z +=
            (mesh.userData.baseRotZ - mesh.rotation.z) * 0.1;
        }
      });

      const targetZoom = isFull
        ? 2.15
        : Math.max(0.4, numWeeksPacked / 20);
      const cw = canvas.clientWidth;
      let targetRadius;
      ...
      if (!isMobileRef.current && !isTabletRef.current) {
        gyroCurrent.current.x +=
          (gyroTarget.current.x - gyroCurrent.current.x) * 0.05;
        gyroCurrent.current.z +=
          (gyroTarget.current.z - gyroCurrent.current.z) * 0.05;
      ...
      if (isRotatingRef.current) {
        targetSpeed.current +=
          (currentDirection.current * baseSpeed - targetSpeed.current) *
          0.05;
        currentSpeed.current +=
          (targetSpeed.current - currentSpeed.current) * 0.1;
  ...
  const cardBg = isDarkMode
    ? "bg-[#161b22]/90 border-zinc-700/60"
      ...
      <div className="relative w-full h-[360px] md:h-[460px] lg:h-[600px]">
        {loading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 z-10">
            ...
            />
            <span className={`text-xs ${hintStyle}`}>
            ...
            <motion.button
              ...
              className={`px-2.5 py-1 md:px-3 md:py-1.5 lg:px-4 lg:py-2 rounded-full text-[9px] md:text-[10px] lg:text-xs font-semibold backdrop-blur-md border shadow-md transition-all cursor-pointer select-none
                ${
                  isDarkMode
                    ? !showEmptyDays
                      ? "bg-[#39d353]/20 text-[#39d353] border-[#39d353]/50 hover:bg-[#39d353]/30"
                      : "bg-[#161b22]/90 text-zinc-400 border-zinc-700/60 hover:text-zinc-200"
                    : !showEmptyDays
                    ? "bg-green-100 text-green-700 border-green-300 hover:bg-green-200"
                    : "bg-white/90 text-gray-500 border-gray-200 hover:text-gray-700"
                }`}
          ...
          <div className="md:hidden absolute bottom-0 inset-x-0 z-10 bg-brand-bg/70 backdrop-blur-lg border-t border-brand-border/30">
            <div className="grid grid-cols-5 divide-x divide-brand-border/30">
              {[
                ...
                >
                  <span
                    className={`text-sm font-bold tabular-nums ${textNumber}`}
                  >
                    {s.val}
                  </span>
                  <span
                    className={`text-[8px] font-medium mt-0.5 ${textSub}`}
                  >
                    {s.label}
                  </span>
                </div>
            ...
            <div className="grid grid-cols-7 divide-x divide-brand-border/30">
              {[
                ...
                { val: stats.avg, label: "Avg/Day", sub: "active days" },
                {
                  ...
                  label: "Longest",
                  sub: stats.longestRange,
                },
                {
                  val: `${stats.currentStreak}d`,
                  label: "Current",
                  sub: stats.currentRange,
                },
                {
                  val: stats.total > 0 ? Math.round(stats.total / 365) : 0,
                  label: "Daily Avg",
                  sub: "per year",
                },
              ].map((s) => (
                ...
                >
                  <span
                    className={`text-base font-bold tabular-nums ${textNumber}`}
                  >
                    {s.val}
                  </span>
                  <span
                    className={`text-[9px] font-semibold mt-0.5 ${textLabel}`}
                  >
                    {s.label}
                  </span>
                  <span
                    className={`text-[8px] mt-0.5 ${textSub} truncate max-w-full`}
                  >
                    {s.sub}
                  </span>
                </div>
        ...
        {stats && !loading && (
          <div className="hidden lg:block">
            ...
            >
              <h3
                className={`text-sm font-semibold mb-2 drop-shadow-md ${textTitle}`}
              >
                Contributions
              </h3>
              <div
                className={`backdrop-blur-md border rounded-xl p-4 shadow-2xl flex gap-6 ${cardBg}`}
              >
                <div>
                  ...
                  <div
                    className={`text-[11px] font-semibold mt-1 ${textLabel}`}
                  >
                    Total
                  </div>
                ...
                <div>
                  ...
                  <div className={`text-[10px] ${textSub}`}>
                    {new Date(stats.bestDate).toLocaleDateString("en-US", {
                      month: "short",
                      day: "numeric",
                    })}
                  </div>
              ...
              <div
                className={`text-right text-[10px] mt-2 font-medium ${textSub}`}
              >
                Average:{" "}
                <span className={`font-bold ${textNumber}`}>{stats.avg}</span>{" "}
                / day
              </div>
            </motion.div>
        ...
        {tooltip && (
          ...
          >
            <span className={`font-bold ${textNumber}`}>
              {tooltip.count} contribution
              {tooltip.count !== 1 ? "s" : ""}
            </span>
            <span
              className={`ml-1.5 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}
            >
              on {tooltip.date}
            </span>
          </div>
        ...
        {!loading && !error && (
          <div
            className={`hidden lg:block absolute bottom-3 left-1/2 -translate-x-1/2 text-[11px] select-none opacity-60 pointer-events-none ${hintStyle}`}
          >
